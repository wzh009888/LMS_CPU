...
const char *default_parm_set = "10/8,5/8";
...
int main(int argc, char **argv) {
    ...
    //     return 0;
    // }
    //第一步，密钥生成(没有交互，也就不支持用户指定种子和i值)

        // const char keyname[] = "Test02";
        const char *parmname = 0;
        char** file = NULL;
        file = (char**)malloc(sizeof(char*) * 2);
        file[0] = (char*)malloc(sizeof(char) * 13);
        file[1] = (char*)malloc(sizeof(char) * 13);
        memcpy(file[0], "Test02", 13);
        memcpy(file[1], "testfile.txt", 13);

        if (!keygen( file[0], parmname )) {
            printf( "Error creating keys\n" );
        }
        //usleep(1000000);
    //第二步，生成签名

        if (!sign( file[0], &file[1] )) {
            printf( "Error signing\n" );
        }
        //usleep(1000000);
    //第三步，签名验证

        if (!verify( file[0], &file[1] )) {
            printf( "Error verifying\n" );
        }
