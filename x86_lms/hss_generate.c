...
MPI_Comm local_comm001;
MPI_Comm local_comm04;
MPI_Comm local_comm08;
MPI_Comm local_comm16;
MPI_Comm local_comm64;
...
#if DO_FLOATING_POINT
static float estimate_total_cost(struct init_order *order, unsigned count_order);
	...
	struct hss_extra_info *info)
{
...
#ifdef Gather06
	if(proc_num > 1){
	    int new_size = proc_num / 1;

		int local_rank001[new_size];   //子通信域中的进程数
		MPI_Group World_Group, local_group;

		MPI_Comm_group(MPI_COMM_WORLD, &World_Group);

		for (int i = me % 1, r = 0; i < proc_num; r++, i += 1) {
			local_rank001[r] = i; //存全局进程号
		}
		MPI_Group_incl(World_Group, new_size, local_rank001, &local_group);
		MPI_Comm_create(MPI_COMM_WORLD, local_group, &local_comm001);
	}
	// if(me == 0) printf("***********1\n");
	if(proc_num > 4){
	    int new_size = proc_num / 4;

		int local_rank04[new_size];   //子通信域中的进程数
		MPI_Group World_Group, local_group;

		MPI_Comm_group(MPI_COMM_WORLD, &World_Group);

		for (int i = me % 4, r = 0; i < proc_num; r++, i += 4) {
			local_rank04[r] = i; //存全局进程号
		}
		MPI_Group_incl(World_Group, new_size, local_rank04, &local_group);
		MPI_Comm_create(MPI_COMM_WORLD, local_group, &local_comm04);
	}
	// if(me == 0) printf("***********4\n");
	//构建3层树，即8个节点，划分子通信域local_comm08 (树高15才有3层树)
	if(proc_num > 8){
	    int new_size = proc_num / 8;

		int local_rank08[new_size];   //子通信域中的进程数
		MPI_Group World_Group, local_group;

		MPI_Comm_group(MPI_COMM_WORLD, &World_Group);

		for (int i = me % 8, r = 0; i < proc_num; r++, i += 8) {
			local_rank08[r] = i; //存全局进程号
		}
		MPI_Group_incl(World_Group, new_size, local_rank08, &local_group);
		MPI_Comm_create(MPI_COMM_WORLD, local_group, &local_comm08);
	}
	// if(me == 0) printf("***********8\n");
	//构建4层树，即16个节点，划分子通信域local_comm16
	if(proc_num > 16){
	    int new_size = proc_num / 16;

		int local_rank16[new_size];   //子通信域中的进程数
		MPI_Group World_Group, local_group;

		MPI_Comm_group(MPI_COMM_WORLD, &World_Group);

		for (int i = me % 16, r = 0; i < proc_num; r++, i += 16) {
			local_rank16[r] = i; //存全局进程号
		}
		MPI_Group_incl(World_Group, new_size, local_rank16, &local_group);
		MPI_Comm_create(MPI_COMM_WORLD, local_group, &local_comm16);
	}
	// if(me == 0) printf("***********16\n");
	//构建6层树，即64个节点，划分子通信域local_comm64
	if(proc_num > 64){

		MPI_Barrier(MPI_COMM_WORLD);

		int new_size = proc_num / 64;

		int local_rank64[new_size];   //子通信域中的进程数
		MPI_Group World_Group, local_group;

		MPI_Comm_group(MPI_COMM_WORLD, &World_Group);

		for (int i = me % 64, r = 0; i < proc_num; r++, i += 64) {
			local_rank64[r] = i; //存全局进程号
		}
		MPI_Group_incl(World_Group, new_size, local_rank64, &local_group);
		MPI_Comm_create(MPI_COMM_WORLD, local_group, &local_comm64);
	}
	// if(proc_num > 64){
	//
	// 	MPI_Barrier(MPI_COMM_WORLD);
	...
	for (i = w->levels - 1; i >= 0; i--) {
		...
		for (j = tree->sublevels - 1; j >= 0; j--) {
			...
			// printf("*************************\n" );
			// printf("h_subtree = %d\n",h_subtree );
			// printf("active->levels_below = %d\n",active->levels_below );
			// printf("size_subtree = %ld\n",size_subtree );
	...
	unsigned num_tracks = 4 * 1;            //hss_thread_num_tracks(info->num_threads);
...
#if DO_FLOATING_POINT
	unsigned core_target = 5 * 1;//hss_thread_num_tracks(info->num_threads);
	float prev_cost = 0;
#endif
	...
	for (i = 0; i < count_order; i++) {
		...
		if (sub) {
			...
			for (n = 0; n < p_order->count_nodes; n++) {
				...
				for (k = 0; k < sub->num_hashes; k++) {
					...
					hss_gen_intermediate_tree(&detail);
				}
			}
			continue;
		}
...

		{
			...
			for (n = 0; n < p_order->count_nodes;) {
				...
				hss_gen_intermediate_tree(&detail);
				n += this_req;
				//    printf("this_req = %ld %ld\n", this_req, detail.node_num);
			}
	...
	// MPI_Comm_free(&local_comm001);
	// MPI_Comm_free(&local_comm04);
	// MPI_Comm_free(&local_comm08);
	// MPI_Comm_free(&local_comm16);
	// MPI_Comm_free(&local_comm64);
	return true;
